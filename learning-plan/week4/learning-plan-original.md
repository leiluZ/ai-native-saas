...
- **实操**：
  1. 部署 `vLLM` (0.7+) 与 `Ollama`，拉取同一基座模型（如 Qwen2.5-7B-Instruct 或 Llama 3.1 8B）。
...
6. 包含 Docker Compose 配置（vllm + ollama 并行）
```
...
6. 模型注册表：{name, provider, endpoint, api_key, priority}
7. 健康检查：定期探测模型可用性，自动降级
```
...
6. 自动扫描 GPU：型号、显存、CUDA 版本
7. 支持热加载：无需重启容器即可应用新参数
```
...
6. 路由规则：VIP 用户 -> GPT-4，普通用户 -> 本地 vLLM
7. 负载均衡：本地实例 CPU/GPU 利用率 <80% 时才路由
8. 成本控制：单用户月度上限 $10，超限降级到本地
9. 提供 /admin/routes 仪表盘：可视化路由决策、成本、延迟
```
...
6. 使用 LLM 分析日志，生成自然语言优化建议
7. 识别瓶颈类型：I/O 密集、计算密集、锁竞争
8. 提供代码级优化建议：具体到函数/行号
9. 输出优化前后对比：{metric_before, metric_after, improvement}
```
...
6. Prometheus 指标：{requests_total, latency_seconds, vram_usage, cache_hit_rate}
7. Grafana 仪表盘：实时监控、告警规则（延迟 >500ms）
8. Kubernetes：HPA 基于 CPU/GPU 利用率自动扩缩
9. 提供 /healthz 端点：K8s liveness/readiness 探测
```
...
1. **推理引擎 Benchmark 自动化脚本 Prompt**
   ...
   - 精确计算 TTFT、TPOT、End-to-End Latency、Peak GPU VRAM
   - 自动适配 vLLM 与 Ollama 双端点
   - 结果导出为 CSV，附性能对比可视化脚本

2. **AWQ/GPTQ/INT8 量化转换与精度验证 Prompt**
   - 支持 AWQ/GPTQ/INT8 三种策略
   - 校准数据集自动下载/加载
   - 量化后自动加载至 vLLM
   - 包含质量回归测试

3. **OpenAI 兼容网关与流式转发代理 Prompt**
   - FastAPI 路由完整映射 chat/completions、models、embeddings
   - 严格遵循 OpenAI 流式协议
   - 支持 Function Calling
   - 集成 API Key 校验、IP 限流、全局超时

4. **vLLM KV Cache 参数网格寻优 Prompt**
   - 网格搜索：gpu_memory_utilization、block_size、max_num_seqs
   - 每组配置自动启动 vLLM，运行 3 轮压测
   - 动态调整 enable_chunked_prefill 与 max_num_batched_tokens
   - 输出最优配置组合与性能收益对比图

5. **本地-云端智能路由与降级策略 Prompt**
   - 主备路由：优先请求本地 vLLM
   - 健康检查：每 5s 探测本地端点
   - 降级触发器：P99 > 800ms、5xx 连续 ≥3、GPU 显存 >95%
   - 流式聚合：fallback 后继续输出剩余 chunk

6. **AI 辅助 Profiling 瓶颈定位与优化 Prompt**
   - 集成 torch.profiler 与 nsys
   - 输出火焰图与阶段耗时分布
   - 自动识别 Top 3 瓶颈
   - 生成优化建议报告

7. **容器化部署 + Prometheus/Grafana 监控集成 Prompt**
   - 多阶段 Dockerfile：仅保留推理运行时依赖
   - docker-compose.yml 编排 vLLM + Prometheus + Grafana
   - Grafana 导入预置 JSON Dashboard
   - Alertmanager 规则：队列积压 >50 或 连续 2min 无响应
...
- **GPU 资源受限**：
  - Day 2 优先使用 `bitsandbytes` INT8 量化，显存占用更低
  - Day 4 降低 `max_num_seqs` 避免 OOM

- **无生产部署经验**：
  - Day 3 先使用 `LiteLLM` 现成网关跑通协议兼容，再逐步替换为自研 FastAPI 代理

- **无监控经验**：
  - Day 7 优先跑通 `vLLM --enable-metrics` + 单节点 Prometheus，Grafana 导入官方模板即可快速可视化

- **算法背景弱**：
  - Day 4/6 聚焦参数调优与 Profiling 报告解读，无需深入 CUDA Kernel 实现，掌握"调参-压测-对比"闭环即可

- **硬件弱 / 模型强**：
  - Day 1-2 放慢节奏，先跑通 vLLM 基础后再做量化

- **硬件强 / 模型弱**：
  - Day 3-5 优先路由策略，更多时间放 Day 6-7 的监控与优化

- **无部署经验**：
  - 先聚焦 Day 1-3 的基础部署，量化与优化放在后三天

- **熟悉 Docker/K8s**：
  - Day 7 可以快速完成，更多时间放 Day 4-6 的性能调优
