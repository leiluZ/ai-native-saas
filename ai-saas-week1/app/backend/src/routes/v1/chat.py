...
async def send_message(request: ChatMessageRequest, db: AsyncSession = Depends(get_db), redis_client: redis.Redis = Depends(get_redis)):
    ...
    try:
        ...
        pass
    return ResponseBase(code=200, message="success", data=ai_response)

@router.get("/history/{session_id}", summary="获取聊天历史", response_model=ResponseBase[ChatHistoryResponse])
async def get_chat_history(session_id: str, redis_client: redis.Redis = Depends(get_redis)):
    if session_id not in session_store:
        try:
    ...
    return ResponseBase(code=200, message="success", data=history)
