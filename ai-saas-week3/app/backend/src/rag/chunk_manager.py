...
class ChunkManager:
    ...
    def chunk_document(
        ...
        if strategy == "fixed":
            ...
            chunks = self._chunk_header_aware(
                content, actual_chunk_size, overlap_tokens
            )
        else:
    ...
    def _chunk_fixed(
        self, content: str, chunk_size: int, overlap_tokens: int
    ) -> List[str]:
        """固定长度分块"""
            ...
            for sep in separators:
                ...
                for i in range(len(tokens) - target_size, target_size // 2, -1):
                    if tokens[i : i + len(sep_tokens)] == sep_tokens:
                        ...
                        right = tokens[i:]
                        return split_tokens(left, target_size) + split_tokens(
                            right, target_size
                        )
